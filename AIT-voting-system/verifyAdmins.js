...
const targets = ['nivya@votehub.com', 'santhosh@votehub.com', 'sanjay@votehub.com', 'dhilip@votehub.com'];
...
async function checkAdmins() {
  ...
  if (snapshot.exists()) {
    ...
    const emails = Object.values(data).map(a => a.email);
    targets.forEach(email => {
      console.log(`${email}: ${emails.includes(email) ? 'FOUND' : 'NOT FOUND'}`);
    });
  } else {
